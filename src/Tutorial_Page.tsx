...
import crm_connected from "./components/assets/crm_connected.png";
import dashboard from "./components/assets/dashboard.png";
import integration_1 from "./components/assets/integration_1.png";
import lead_connected from "./components/assets/lead_connected.png";
import lead_source from "./components/assets/lead_source.png";
import leads from "./components/assets/leads.png";
import password from "./components/assets/passsword.png";
import sessions from "./components/assets/sessions.png";
import sync from "./components/assets/sync.png";
import test_connection from "./components/assets/test_connection.png";
import threads from "./components/assets/threads.png";
import three_crm_updates from "./components/assets/3_crm_update.png";
import zoho_crm from "./components/assets/zoho_crm.png";
import hubspot_connected from "./components/assets/hubspot_connected.png";
import hubspot_crm from "./components/assets/hubspot_crm.png";
import salesforce_crm from "./components/assets/salesforce_crm.png";
import salesforce_connected from "./components/assets/salesforce_connected.png";
...
const tutorialSections: Array<{ sectionId: number; sectionTitle: string; sectionTag: string; sectionDescription: string; sectionColor: string; steps: TutorialStep[] }> = [
  {
    sectionId: 1,
    sectionTitle: "1. Registration & Sign-In",
    sectionTag: "Getting Started",
    sectionDescription: "Register, verify & sign in to access the platform",
    sectionColor: "#3B82F6",
    steps: [
      { number: 1, title: "Create Account & Register", description: "Register with your email/password or Google to get started with InteGrow System.", icon: UserPlus, iconColor: "#3B82F6", image: create_account, details: ["Enter full name, email, and a strong password","Password must include uppercase, lowercase, and a number","Or use Sign in with Google for instant access","Already have an account? Tap Sign In"] },
      { number: 2, title: "Sign In", description: "Sign in securely with your credentials or Google account.", icon: LogIn, iconColor: "#10B981", image: sign_in, details: ["Enter registered email and password, or use Google","Success toast confirms redirect after account creation","Tap Forgot Password to reset if you can't sign in","Ensure your email is verified to access all features"] },
      { number: 3, title: "Forgot Password", description: "Enter your email to receive a password reset link.", icon: KeyRound, iconColor: "#F59E0B", image: forgot_password, details: ["Tap Forgot Password on the Sign In screen","Enter your registered email address","Tap Send Reset Link and check your inbox","Check spam/junk if the email doesn't arrive"] },
      { number: 4, title: "Dashboard Overview", description: "View key pipeline stats and a 7-day lead activity trend.", icon: LayoutDashboard, iconColor: "#EC4899", image: dashboard, details: ["Leads (7 Days) — leads fetched this week","Pushed (7 Days) — leads successfully sent to your CRM","Pending Queue — fetched but not yet pushed","Activity chart — hover any point to see the daily count"] },
    ],
  },
  {
    sectionId: 2,
    sectionTitle: "2. CRM Integration Setup",
    sectionTag: "Integration",
  ...
  {
    sectionId: 3,
    sectionTitle: "3. Leads Management",
    sectionTag: "Leads",
    sectionDescription: "Track and export all synced IndiaMART leads",
    sectionColor: "#10B981",
    steps: [
      { number: 12, title: "Configure Lead Source", description: "Scroll down on the Integrations page to find the Lead Source section — enter your IndiaMART API Key to go live, or use Mock Leads for testing.", icon: Globe, iconColor: "#06B6D4", image: lead_source, details: ["Enter IndiaMART API Key and click Configure & Enable","Mock Leads is active by default — no API key needed","Disable Mock Leads before enabling IndiaMART","Sync Status panel shows active CRM and last sync time"] },
      { number: 13, title: "Leads in Bitrix CRM", description: "Synced IndiaMART leads appear as cards in your Bitrix24 CRM pipeline.", icon: CheckCircle, iconColor: "#16A34A", image: lead_connected, details: ["Leads appear under Lead Entry column in the Kanban board","Each card shows lead name, source, responsible person, and modified date","Pipeline stages: Lead Entry → Calling → Meeting → Data Gathering → Quotation","Confirms IndiaMART → InteGrow → Bitrix sync is working end to end"] },
      { number: 14, title: "Leads Screen", description: "View, filter, and search all fetched leads.", icon: Users, iconColor: "#F97316", image: leads, details: ["Filter by status: All, Pending, Pushed, or Failed","Search by name, phone, email, or product","Filter by date range using From / To pickers","Thread column links each lead to its customer thread"] },
    ],
  },
  {
    sectionId: 4,
    sectionTitle: "4. Sync & Threads",
    sectionTag: "Synchronization",
    sectionDescription: "Push leads to your CRM and browse them grouped by customer",
    sectionColor: "#8B5CF6",
    steps: [
      { number: 15, title: "Sync Engine", description: "Run Sync manually or review all sync history in Sync Logs.", icon: Database, iconColor: "#8B5CF6", image: sync, details: ["Click Run Sync to immediately push all pending leads","Most Recent Sync shows: Fetched, Pushed, Skipped, Failed","Sync Logs table lists all runs — filter by All, Pushed, or Failed","Skipped = duplicate leads already present in your CRM"] },
      { number: 16, title: "Lead Threads", description: "All leads from the same customer are grouped into one thread.", icon: MessageSquare, iconColor: "#A855F7", image: threads, details: ["Thread card shows: name, phone, email, and lead count","Last activity date displayed on each thread card","Click a thread to see all associated leads in order","Leads table links back via the Thread / View column"] },
    ],
  },
  {
    sectionId: 5,
    sectionTitle: "5. Account Settings",
    sectionTag: "Settings",
    sectionDescription: "Manage profile, security, and sessions",
    sectionColor: "#64748B",
    steps: [
      { number: 17, title: "Profile Settings", description: "Update your name and phone. Email is read-only.", icon: Shield, iconColor: "#64748B", image: account_settings, details: ["Edit Full Name and Phone number","Click Save Profile to apply changes","Email is locked and cannot be changed","Changes reflect immediately across the platform"] },
      { number: 18, title: "Change Password", description: "Update your password under the Security section.", icon: KeyRound, iconColor: "#D97706", image: password, details: ["Enter Current Password to verify your identity","Enter New Password and Confirm Password","Must contain uppercase, lowercase, and a number","Click Update Password to save changes"] },
      { number: 19, title: "Sessions & Delete Account", description: "View active sessions or permanently delete your account.", icon: Shield, iconColor: "#EF4444", image: sessions, details: ["Sessions panel lists all active logins per device","Current session is labelled 'Active Now'","Toggle 'I understand this cannot be undone' to enable Delete","Deletion permanently removes all leads, threads, and sync data"] },
    ],
  },
];
      ...
      <div style={{ position: 'relative', width: '100%', overflow: 'hidden', borderRadius: '14px' }}>
        <AnimatePresence mode="popLayout" custom={direction}>
          <motion.div
            ...
            custom={direction}
            variants={{
              enter:  (dir: number) => ({ opacity: 0, x: dir * 120, scale: 0.94 }),
              center: { opacity: 1, x: 0, scale: 1 },
              exit:   (dir: number) => ({ opacity: 0, x: dir * -120, scale: 0.94 }),
            }}
          ...
          >
            ...
            <motion.div
              ...
              style={{ position: 'absolute', inset: 0, borderRadius: '14px', background: `linear-gradient(135deg, ${iconColor}28, rgba(0,0,0,0.45))`, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'zoom-in', backdropFilter: 'blur(3px)', WebkitBackdropFilter: 'blur(3px)' }}
            >
              <div style={{ background: 'rgba(0,0,0,0.62)', borderRadius: '50%', padding: '14px', border: `2.5px solid ${iconColor}`, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 0 24px rgba(0,0,0,0.4)' }}>
                <ZoomIn size={26} color={iconColor} strokeWidth={2} />
              </div>
            </motion.div>
          </motion.div>
        </AnimatePresence>
        ...
        {images.length > 1 && (
          <>
            <motion.button onClick={() => go(-1)} whileHover={{ scale: 1.12 }} whileTap={{ scale: 0.9 }}
              style={{ position: 'absolute', left: '8px', top: '50%', transform: 'translateY(-50%)', width: '36px', height: '36px', borderRadius: '50%', background: 'rgba(255,255,255,0.92)', border: `1.5px solid ${iconColor}35`, boxShadow: '0 4px 14px rgba(0,0,0,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', zIndex: 3 }}>
              <ChevronLeft size={16} color={iconColor} strokeWidth={2.5} />
            </motion.button>
            <motion.button onClick={() => go(1)} whileHover={{ scale: 1.12 }} whileTap={{ scale: 0.9 }}
              style={{ position: 'absolute', right: '8px', top: '50%', transform: 'translateY(-50%)', width: '36px', height: '36px', borderRadius: '50%', background: 'rgba(255,255,255,0.92)', border: `1.5px solid ${iconColor}35`, boxShadow: '0 4px 14px rgba(0,0,0,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', zIndex: 3 }}>
              <ChevronRight size={16} color={iconColor} strokeWidth={2.5} />
            </motion.button>
          </>
        )}
      </div>
    ...
    <div style={{ minHeight: '100vh', fontFamily: '"Inter", sans-serif', background: 'linear-gradient(180deg, #ecfeff 0%, #f8fafc 30%, #ffffff 100%)', position: 'relative', width: '100%', overflowX: 'hidden' }}>
      ...
      <motion.div animate={{ marginLeft: (!isMobile && sidebarOpen) ? 272 : 0 }} transition={{ type: 'spring', stiffness: 340, damping: 34 }}>
        ...
        <section style={{ padding: isMobile ? '2.5rem 1rem 1.5rem' : '3rem 1.5rem 2rem', background: 'linear-gradient(to bottom, rgba(255,255,255,0), #f8fafc)' }}>
          <div style={{ maxWidth: '1280px', margin: '0 auto', textAlign: 'center', padding: isMobile ? '0 0.5rem' : '0 1.5rem' }}>
            ...
            <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} viewport={{ once: true }} style={{ fontFamily: '"Inter", sans-serif', fontSize: isMobile ? '15px' : '17px', color: BRAND.slate, margin: '0 auto', lineHeight: isMobile ? '23px' : '26px', fontWeight: 400 }}>Master InteGrow with our comprehensive guide covering every feature from sign-up to advanced functionality</motion.p>
            <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.35 }} viewport={{ once: true }} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: '0.75rem', padding: isMobile ? '0.65rem 1rem' : '0.75rem 1.25rem', margin: '1.25rem auto 0', maxWidth: 'fit-content', background: `linear-gradient(135deg, ${BRAND.primary}10, rgba(59,130,246,0.10))`, border: `1.5px solid ${BRAND.primary}30`, borderRadius: '999px', backdropFilter: 'blur(14px)', WebkitBackdropFilter: 'blur(14px)', boxShadow: `0 4px 12px ${BRAND.primary}12, inset 0 1px 0 rgba(255,255,255,0.6)` }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: isMobile ? '2rem' : '2.25rem', height: isMobile ? '2rem' : '2.25rem', borderRadius: '8px', background: `linear-gradient(135deg, ${BRAND.primary}30, ${BRAND.primary}18)`, border: `1.5px solid ${BRAND.primary}40`, flexShrink: 0 }}><ArrowLeft size={isMobile ? 14 : 16} color={BRAND.primaryDk} strokeWidth={2.5} /></div>
              <span style={{ fontFamily: '"Inter", sans-serif', fontSize: isMobile ? '0.875rem' : isTablet ? '0.9375rem' : '1rem', color: BRAND.primaryDk, fontWeight: 600, letterSpacing: '-0.01em' }}>{isMobile ? 'Use buttons to navigate steps' : 'Use arrow keys to navigate between steps'}</span>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: isMobile ? '2rem' : '2.25rem', height: isMobile ? '2rem' : '2.25rem', borderRadius: '8px', background: `linear-gradient(135deg, ${BRAND.primary}30, ${BRAND.primary}18)`, border: `1.5px solid ${BRAND.primary}40`, flexShrink: 0 }}><ArrowRight size={isMobile ? 14 : 16} color={BRAND.primaryDk} strokeWidth={2.5} /></div>
            </motion.div>
          </div>
        </section>
          ...
          <div style={{ maxWidth: '1200px', margin: '0 auto', padding: isMobile ? '0.75rem 1rem' : '0.85rem 2rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
            <motion.button onClick={() => setSidebarOpen(o => !o)} whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }} style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '5px 12px 5px 7px', background: sidebarOpen ? `linear-gradient(135deg, ${BRAND.primary}18, rgba(56,189,248,0.12))` : 'linear-gradient(135deg, rgba(15,23,42,0.06), rgba(15,23,42,0.03))', border: `1.5px solid ${sidebarOpen ? BRAND.primary + '45' : 'rgba(0,0,0,0.1)'}`, borderRadius: '999px', backdropFilter: 'blur(10px)', cursor: 'pointer', flexShrink: 0, transition: 'all 0.22s ease' }}>
              <div style={{ width: '26px', height: '26px', borderRadius: '7px', flexShrink: 0, background: sidebarOpen ? `linear-gradient(135deg, ${BRAND.primary}30, ${BRAND.primary}18)` : 'linear-gradient(135deg, rgba(15,23,42,0.1), rgba(15,23,42,0.06))', border: `1.5px solid ${sidebarOpen ? BRAND.primary + '40' : 'rgba(0,0,0,0.12)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '1px' }}>
                {sidebarOpen ? <><ChevronLeft size={11} color={BRAND.primaryDk} strokeWidth={2.5} /><div style={{ width: '1.5px', height: '10px', borderRadius: '2px', background: BRAND.primaryDk }} /></> : <><div style={{ width: '1.5px', height: '10px', borderRadius: '2px', background: BRAND.slate }} /><ChevronRight size={11} color={BRAND.slate} strokeWidth={2.5} /></>}
              </div>
            ...
            <div style={{ flex: 1 }}><ProgressBar current={globalIdx} total={TOTAL_STEPS} color={currentStep?.iconColor || BRAND.primary} /></div>
            ...
            {!isMobile && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0 }}>
                {[{ id: 'step', Icon: List, label: 'Step view' }, { id: 'overview', Icon: Grid3X3, label: 'Overview' }].map(({ id, Icon: Ic, label }) => {
                  ...
                  return <motion.button key={id} onClick={() => setViewMode(id as 'step' | 'overview')} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} style={{ display: 'inline-flex', alignItems: 'center', gap: '0.4rem', padding: '5px 11px 5px 7px', background: active ? `linear-gradient(135deg, ${BRAND.primary}18, rgba(56,189,248,0.12))` : 'linear-gradient(135deg, rgba(15,23,42,0.05), rgba(15,23,42,0.02))', border: `1.5px solid ${active ? BRAND.primary + '45' : 'rgba(0,0,0,0.09)'}`, borderRadius: '999px', backdropFilter: 'blur(10px)', cursor: 'pointer', transition: 'all 0.2s ease' }}>
                    <div style={{ width: '22px', height: '22px', borderRadius: '6px', flexShrink: 0, background: active ? `linear-gradient(135deg, ${BRAND.primary}30, ${BRAND.primary}18)` : 'linear-gradient(135deg, rgba(15,23,42,0.09), rgba(15,23,42,0.05))', border: `1.5px solid ${active ? BRAND.primary + '40' : 'rgba(0,0,0,0.1)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Ic size={12} color={active ? BRAND.primaryDk : BRAND.slate} strokeWidth={2} /></div>
              ...
              <AnimatePresence mode="wait">
                <motion.div key={`section-${currentSection?.sectionId}`} initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }} transition={{ duration: 0.3 }} style={{ textAlign: 'center', width: '100%', maxWidth: '700px' }}>
                  <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '4px 14px', borderRadius: '999px', marginBottom: '0.6rem', background: `${currentStep?.iconColor}14`, border: `1.5px solid ${currentStep?.iconColor}35` }}>
                    ...
                    <span style={{ fontSize: '0.68rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: currentStep?.iconColor, fontFamily: '"Inter", sans-serif' }}>{currentSection?.sectionTitle}</span>
                  </div>
                  <p style={{ fontSize: isMobile ? '13px' : '14px', color: BRAND.muted, margin: 0, lineHeight: 1.65, fontFamily: '"Inter", sans-serif' }}>{currentSection?.sectionDescription}</p>
                </motion.div>
              </AnimatePresence>
            </div>
                ...
                <motion.div key={section.sectionId} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-40px' }} transition={{ duration: 0.4, ease: 'easeOut' }} style={{ marginBottom: isMobile ? '3.5rem' : '5rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem', paddingBottom: '1.25rem', borderBottom: '2px solid rgba(0,0,0,0.07)' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '7px', flexShrink: 0 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <span style={{ fontSize: '10px', color: BRAND.muted, fontFamily: '"Inter", sans-serif' }}>{section.steps.filter((st: any) => ALL_STEPS.findIndex((s: any) => s.number === st.number) < globalIdx).length}/{section.steps.length} done</span>
                        <div style={{ width: '80px', height: '5px', background: 'rgba(0,0,0,0.07)', borderRadius: '5px', overflow: 'hidden' }}>
                          <div style={{ height: '100%', borderRadius: '5px', background: `linear-gradient(to right, ${sectionColor}, ${sectionColor}aa)`, width: `${(section.steps.filter((st: any) => ALL_STEPS.findIndex((s: any) => s.number === st.number) < globalIdx).length / section.steps.length) * 100}%`, transition: 'width 0.4s ease' }} />
                        </div>
                      </div>
                    </div>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(2, 1fr)', gap: isMobile ? '1.25rem' : '1.5rem' }}>
                    {section.steps.map((step: any, stepLocalIdx: number) => {
                      const stepGlobalIdx = ALL_STEPS.findIndex((s: any) => s.number === step.number)
                      const Icon = step.icon; const isPast = stepGlobalIdx < globalIdx; const isCurrent = stepGlobalIdx === globalIdx
                      const hasMultiImg = Array.isArray(step.images) && step.images.length > 1
                      return (
                        <motion.button key={step.number} id={`overview-step-${stepGlobalIdx}`} onClick={() => { setGlobalIdx(stepGlobalIdx); setViewMode('step') }} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.3, delay: stepLocalIdx * 0.04, ease: 'easeOut' }} whileHover={{ y: -6, boxShadow: `0 20px 48px ${step.iconColor}28, 0 4px 12px rgba(0,0,0,0.08)`, transition: { duration: 0.18 } }} whileTap={{ scale: 0.985, transition: { duration: 0.1 } }}
                          ...
                          <div style={{ padding: isMobile ? '1.5rem 1.5rem 1.5rem 1.25rem' : '1.75rem 1.75rem 1.75rem 1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem', flex: 1 }}>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                {hasMultiImg && <div style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '3px 8px', borderRadius: '999px', background: `${step.iconColor}12`, border: `1px solid ${step.iconColor}30` }}><div style={{ width: '5px', height: '5px', borderRadius: '50%', background: step.iconColor }} /><span style={{ fontSize: '9px', fontWeight: 700, color: step.iconColor, fontFamily: '"Inter", sans-serif' }}>{step.images.length} slides</span></div>}
      ...
      <div style={{ padding: isMobile ? '2.5rem 1.25rem 3rem' : '1rem 1.5rem', textAlign: 'center', background: 'linear-gradient(135deg, #e8f4fd 0%, #f0f9ff 40%, #eef2ff 100%)', borderTop: '1px solid rgba(99,179,237,0.2)', position: 'relative', overflow: 'hidden' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '5px 14px', background: 'rgba(255,255,255,0.75)', backdropFilter: 'blur(8px)', border: '1px solid rgba(99,179,237,0.35)', borderRadius: '999px', fontSize: '12px', fontWeight: 600, color: '#0369a1', marginBottom: '1.25rem', letterSpacing: '0.03em', boxShadow: '0 2px 8px rgba(99,179,237,0.15)' }}>
          <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#22c55e', boxShadow: '0 0 0 2px rgba(34,197,94,0.3)', display: 'inline-block' }} /> All systems ready
        </motion.div>
        ...
        <motion.p initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} style={{ fontSize: '15px', color: BRAND.slate, maxWidth: '380px', margin: '0 auto 1.75rem', lineHeight: 1.65 }}>You've covered the essentials. Head to the Dashboard to put InteGrow to work.</motion.p>
        <motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }}>
          <motion.button onClick={() => window.location.href = 'https://integrow.org/'} whileHover={{ scale: 1.04, y: -2, boxShadow: '0 16px 40px rgba(15,23,42,0.35)' }} whileTap={{ scale: 0.97 }}
            ...
            <motion.span animate={{ x: [0, 3, 0] }} transition={{ duration: 1.6, repeat: Infinity, ease: 'easeInOut' }} style={{ position: 'relative', zIndex: 1, display: 'flex' }}><ArrowRight size={18} strokeWidth={2.5} /></motion.span>
          </motion.button>
        </motion.div>
      </div>
