...
//import logoImage from "./assets/workeye_1.png";
...
//import leads_1 from "./components/assets/leads_1.png";
...
const tutorialSections: Array<{ sectionId: number; sectionTitle: string; sectionTag: string; sectionDescription: string; sectionColor: string; steps: TutorialStep[] }> = [
    ...
    steps: [
      {
        ...
        description: "Register with your email/password or Google to get started with InteGrow System.",
        icon: UserPlus,
        iconColor: "#3B82F6",
      ...
      },
      {
        number: 2,
        title: "Sign In",
        description: "Sign in securely with your credentials or Google account.",
        icon: LogIn,
        iconColor: "#10B981",
        image: sign_in,
        details: [
          "Enter registered email and password, or use Google",
          "Success toast confirms redirect after account creation",
          "Tap Forgot Password to reset if you can't sign in",
      ...
      },
      {
        ...
        details: [
          ...
          "Tap Send Reset Link and check your inbox",
          "Check spam/junk if the email doesn't arrive",
        ],
      },
      {
        number: 4,
        title: "Dashboard Overview",
        description: "View key pipeline stats and a 7-day lead activity trend.",
        icon: LayoutDashboard,
        iconColor: "#EC4899",
        image: dashboard,
        details: [
          "Leads (7 Days) — leads fetched this week",
          "Pushed (7 Days) — leads successfully sent to your CRM",
          "Pending Queue — fetched but not yet pushed",
          "Activity chart — hover any point to see the daily count",
        ],
      },
    ],
  },
  {
    ...
    steps: [
      {
        number: 5,
        title: "CRM Integrations Screen",
        description: "Connect external CRM systems for lead synchronization.",
        icon: Plug,
        iconColor: "#6366F1",
        image: integration_1,
        details: [
          "Active CRM panel shows connected CRM or 'No CRM connected yet'",
          "Available CRMs: BITRIX, ZOHO, HUBSPOT, SALESFORCE",
          "Each CRM shows ACTIVE or INACTIVE status",
          "Tap any CRM row to open its configuration",
        ],
      },
      {
        number: 6,
        ...
        details: [
          "Paste the inbound Webhook URL",
          "Click Test Connection to verify the webhook",
          "Click Save & Activate to make this CRM the active destination",
          "Use Back to Integrations to cancel and return",
        ],
      },
      {
        number: 7,
        ...
        details: [
          "Green toast: 'Connection successful — Credentials are valid'",
          "If failed, verify the webhook URL is correct and publicly accessible",
          "After a successful test, click Save & Activate to go live",
        ],
      },
      {
        number: 8,
        ...
        image: crm_connected,
        details: [
          "Green CONNECTED badge confirms the active CRM",
          "All new leads automatically sync to this CRM",
          "Click Revoke to disconnect the current CRM & activate new one",
  ...
  },
  {
    ...
    steps: [
      {
        number: 9,
        title: "Configure Lead Source",
        description: "Scroll down on the Integrations page to find the Lead Source section — enter your IndiaMART API Key to go live, or use Mock Leads for testing.",
        icon: Globe,
        iconColor: "#06B6D4",
        image: lead_source,
        details: [
          "Enter IndiaMART API Key and click Configure & Enable",
          "Mock Leads is active by default — no API key needed",
          "Disable Mock Leads before enabling IndiaMART",
          "Sync Status panel shows active CRM and last sync time",
        ],
        
      },
      {
        number: 10,
        title: "Leads in Bitrix CRM",
        description: "Synced IndiaMART leads appear as cards in your Bitrix24 CRM pipeline.",
        icon: CheckCircle,
        iconColor: "#16A34A",
        image: lead_connected,
        details: [
          "Leads appear under Lead Entry column in the Kanban board",
          "Each card shows lead name, source, responsible person, and modified date",
          "Pipeline stages: Lead Entry → Calling → Meeting → Data Gathering → Quotation",
          "Confirms IndiaMART → InteGrow → Bitrix sync is working end to end",
        ],
      },
      {
        number: 11,
        title: "Leads Screen",
        description: "View, filter, and search all fetched leads.",
        icon: Users,
        iconColor: "#F97316",
        image: leads,
        details: [
          "Filter by status: All, Pending, Pushed, or Failed",
          "Search by name, phone, email, or product",
          "Filter by date range using From / To pickers",
          "Thread column links each lead to its customer thread",
        ],
      },
    ],
  },
    ...
    steps: [
      {
        number: 12,
        title: "Sync Engine",
        description: "Run Sync manually or review all sync history in Sync Logs.",
        icon: Database,
        iconColor: "#8B5CF6",
        image: sync,
        details: [
          "Click Run Sync to immediately push all pending leads",
          "Most Recent Sync shows: Fetched, Pushed, Skipped, Failed",
          "Sync Logs table lists all runs — filter by All, Pushed, or Failed",
          "Skipped = duplicate leads already present in your CRM",
        ],
      },
      {
        number: 13,
        title: "Lead Threads",
        description: "All leads from the same customer are grouped into one thread.",
        icon: MessageSquare,
        iconColor: "#A855F7",
        image: threads,
        details: [
          "Thread card shows: name, phone, email, and lead count",
          "Last activity date displayed on each thread card",
          "Click a thread to see all associated leads in order",
          "Leads table links back via the Thread / View column",
        ],
      },
    ],
  },
    ...
    steps: [
      {
        ...
        description: "Update your name and phone. Email is read-only.",
        icon: Shield,
        iconColor: "#64748B",
        image: account_settings,
        details: [
      ...
      },
      {
        ...
        description: "Update your password under the Security section.",
        icon: KeyRound,
        iconColor: "#D97706",
        image: password,
        details: [
      ...
      {
        number: 16,
        title: "Sessions & Delete Account",
        description: "View active sessions or permanently delete your account.",
        icon: Shield,
        iconColor: "#EF4444",
        image: sessions,
        details: [
          "Sessions panel lists all active logins per device",
          "Current session is labelled 'Active Now'",
          "Toggle 'I understand this cannot be undone' to enable Delete",
          "Deletion permanently removes all leads, threads, and sync data",
        ],
...
const IndexSidebar = ({ activeGlobalIdx, onJump, isOpen, onToggle, footerOffset }: IndexSidebarProps) => {
  ...
  const [openSections, setOpenSections] = useState<Record<number, boolean>>(() => {
    ...
    const active = tutorialSections.find(s =>
      s.steps.some((st: any) => ALL_STEPS.findIndex((x: any) => x.number === st.number) === activeGlobalIdx)
    )
        ...
        >
          ...
          <div style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', scrollbarWidth: 'none', msOverflowStyle: 'none', padding: '0.5rem 0 1rem' }}>
            ...
            {tutorialSections.map((section, secIdx) => {
              ...
              const completedInSection = section.steps.filter((st: any) => ALL_STEPS.findIndex((x: any) => x.number === st.number) < activeGlobalIdx).length
...
export default function TutorialPage() {
  ...
  const prev = useCallback(() => {
    if (canPrev) { setIsUserNavigation(true); setGlobalIdx(i => i - 1) }
  }, [canPrev])

  const next = useCallback(() => {
    if (canNext) { setIsUserNavigation(true); setGlobalIdx(i => i + 1) }
  }, [canNext])
  ...
  useEffect(() => {
    const updateOffset = () => {
      ...
      const footerRect = (footerRef.current as HTMLElement).getBoundingClientRect()
  ...
  useEffect(() => {
    ...
    const elementTop = (contentRef.current as HTMLElement).getBoundingClientRect().top + window.pageYOffset
    ...
    <div style={{ minHeight: '100vh', fontFamily: '"Inter", sans-serif', background: 'linear-gradient(180deg, #ecfeff 0%, #f8fafc 30%, #ffffff 100%)', position: 'relative', width: '100%', overflowX: 'hidden' }}>
          ...
          <div style={{ maxWidth: '1280px', margin: '0 auto', width: '100%', padding: isMobile ? '0 0.5rem' : '0 1.5rem' }}>
            <div style={{ display: isMobile ? 'flex' : 'grid', flexDirection: isMobile ? 'column' : undefined, gridTemplateColumns: isMobile ? undefined : isTablet ? '1fr' : '1.1fr 0.9fr', gap: isMobile ? '2rem' : isTablet ? '2.5rem' : '3rem', alignItems: isMobile ? 'start' : 'center' }}>
              ...
              <div style={{ maxWidth: isMobile ? '100%' : '650px', marginTop: isMobile ? '0' : '-2.5rem' }}>
                ...
                >
                  <span style={{ color: BRAND.primary, fontWeight: 900 }}>Explore Intro IndiaMart</span>{' '}
        ...
        <section style={{ padding: isMobile ? '2.5rem 1rem 1.5rem' : '3rem 1.5rem 2rem', background: 'linear-gradient(to bottom, rgba(255,255,255,0), #f8fafc)' }}>
          <div style={{ maxWidth: '1280px', margin: '0 auto', textAlign: 'center', padding: isMobile ? '0 0.5rem' : '0 1.5rem' }}>
            ...
            <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} viewport={{ once: true }} style={{ fontFamily: '"Inter", sans-serif', fontSize: isMobile ? '15px' : '17px', color: BRAND.slate, margin: '0 auto', lineHeight: isMobile ? '23px' : '26px', fontWeight: 400 }}>
              Master Intro IndiaMart with our comprehensive guide covering every feature from sign-up to advanced functionality
            </motion.p>
            <motion.div
          ...
          <div style={{ maxWidth: '1200px', margin: '0 auto', padding: isMobile ? '0.75rem 1rem' : '0.85rem 2rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
            <motion.button onClick={() => setSidebarOpen(o => !o)} whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }} style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '5px 12px 5px 7px', background: sidebarOpen ? `linear-gradient(135deg, ${BRAND.primary}18, rgba(56,189,248,0.12))` : 'linear-gradient(135deg, rgba(15,23,42,0.06), rgba(15,23,42,0.03))', border: `1.5px solid ${sidebarOpen ? BRAND.primary + '45' : 'rgba(0,0,0,0.1)'}`, borderRadius: '999px', backdropFilter: 'blur(10px)', cursor: 'pointer', flexShrink: 0, transition: 'all 0.22s ease' }}>
              <div style={{ width: '26px', height: '26px', borderRadius: '7px', flexShrink: 0, background: sidebarOpen ? `linear-gradient(135deg, ${BRAND.primary}30, ${BRAND.primary}18)` : 'linear-gradient(135deg, rgba(15,23,42,0.1), rgba(15,23,42,0.06))', border: `1.5px solid ${sidebarOpen ? BRAND.primary + '40' : 'rgba(0,0,0,0.12)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '1px' }}>
                {sidebarOpen ? (
                  <><ChevronLeft size={11} color={BRAND.primaryDk} strokeWidth={2.5} /><div style={{ width: '1.5px', height: '10px', borderRadius: '2px', background: BRAND.primaryDk }} /></>
                ) : (
                  <><div style={{ width: '1.5px', height: '10px', borderRadius: '2px', background: BRAND.slate }} /><ChevronRight size={11} color={BRAND.slate} strokeWidth={2.5} /></>
                )}
              </div>
            ...
            <div style={{ width: '100%', maxWidth: '1160px' }}>
              <AnimatePresence mode="wait">
                <StepCard
                  ...
                  setIsUserNavigation={setIsUserNavigation}
                />
              </AnimatePresence>
            </div>
          </section>
      ...
      <div style={{ padding: isMobile ? '2.5rem 1.25rem 3rem' : '1rem 1.5rem', textAlign: 'center', background: 'linear-gradient(135deg, #e8f4fd 0%, #f0f9ff 40%, #eef2ff 100%)', borderTop: '1px solid rgba(99,179,237,0.2)', position: 'relative', overflow: 'hidden' }}>
        ...
        <motion.p initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} style={{ fontSize: '15px', color: BRAND.slate, maxWidth: '380px', margin: '0 auto 1.75rem', lineHeight: 1.65 }}>
          You've covered the essentials. Head to the Dashboard to put Intro IndiaMart to work.
        </motion.p>
        <motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }}>
          <motion.button
            onClick={() => window.location.href = 'https://integrow.org/'}
